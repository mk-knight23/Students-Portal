...
export default function StudentDashboard() {
    ...
    // Probability Logic
    const prob = student.neet_score > 600 ? 98 : student.neet_score > 500 ? 85 : 62;
                ...
                <div className="space-y-6 relative z-10">
                    <div className="flex items-center gap-5">
                        ...
                        <div className="space-y-0.5 text-left">
                            <h1 className="text-3xl md:text-4xl font-black tracking-tight italic leading-none">
                                {student.name.split(' ')[0]} <span className="text-primary tracking-[-0.05em]">HUB</span>
                            </h1>
                            <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-40 flex items-center gap-2 mt-1">
                                Operational Status <span className="h-1 w-1 rounded-full bg-green-500 animate-pulse" /> {student.status}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                        <div className="px-5 py-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl">
                            ...
                            <p className="text-xl font-black italic">{student.neet_score}</p>
                        </div>
                        <div className="px-5 py-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl">
                            <p className="text-[8px] font-bold uppercase text-white/40 tracking-widest">National Rank</p>
                            <p className="text-xl font-black italic">#{student.neet_rank.toLocaleString()}</p>
                        </div>
                        <div className="px-5 py-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl text-left">
                            ...
                            <p className="text-sm font-black italic uppercase tracking-wider">{student.city}, MH</p>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div className="p-10 rounded-[2.5rem] bg-white border border-black/5 shadow-sm space-y-6">
                                    <div className="space-y-6">
                                        <div className="flex items-center justify-between border-b border-black/5 pb-4">
                                            ...
                                            <span className="text-sm font-black uppercase tracking-tight">{student.academic_history?.class10?.board || "CBSE"} SECURED</span>
                                        </div>
                                        <div className="flex items-center justify-between border-b border-black/5 pb-4">
                                            <span className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">12th Percentage</span>
                                            <span className="text-sm font-black italic">{student.academic_history?.class12?.percentage.toFixed(2)}%</span>
                                        </div>
