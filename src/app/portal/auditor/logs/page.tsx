"use client"

import { StudentMasterTable } from "@/features/admin/components/student-master-table"
import { ShieldCheck, Eye } from "lucide-react"

export default function AuditorLogsPage() {
    return (
        <div className="space-y-8 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 text-purple-500 w-fit border border-purple-500/20">
                        <Eye className="h-3 w-3" />
                        <span className="text-[10px] font-black uppercase tracking-[0.2em]">Audit Node</span>
                    </div>
                    <h1 className="text-4xl font-black tracking-tighter sm:text-5xl uppercase">
                        System <span className="text-purple-500 italic">Audit Logs</span>
                    </h1>
                    <p className="text-muted-foreground font-medium max-w-xl">
                        Comprehensive immutable records of all student interactions, document changes, and payment transitions for compliance verification.
                    </p>
                </div>
            </div>

            <StudentMasterTable />
        </div>
    )
}
